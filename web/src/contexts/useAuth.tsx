...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  // para buscar os dados do usuário caso já esteja logado
  useEffect(() => {
    const token = localStorage.getItem('@dowhile:token')

    if (token) {/* para enviar o token junto com o cabeçalho numa requisição. toda requisição que parte dessa linha terá o token*/
      api.defaults.headers.common.authorization = `Bearer ${token}`
  ...
  return (/*o provider faz com que todos os componentes dentro dele tenham acesso a informação do contexto  */
